										...
										<TABLE id=grid
											...
											<TBODY>
												...
												<TR
													...
													<TD>
														<a href='<c:url value='customer_del.action?cid=${customer.cid }'></c:url>'>删除</a>&nbsp;
														<a href='<c:url value='customer_editPre.action?cid=${customer.cid }'></c:url>'>修改</a>&nbsp;
													</TD>
												</TR>
								...
								<TR>
									<TD><SPAN id=pagelink>
											<DIV
												...
												共[<B>${pb.tr}</B>]条记录,共[<B>${pb.tp}</B>]页
												,当前第[<b>${pb.pc}</b>]页
											<c:if test="${pb.pc > 1 }">
												[<A href="${pb.url }?pc=${pb.pc-1}">前一页</A>]
											</c:if>
											<c:if test="${pb.pc<pb.tp }">
												[<A href="${pb.url }?pc=${pb.pc+1}">后一页</A>] 
											</c:if>	
